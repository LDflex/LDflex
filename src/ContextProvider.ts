...
export default class ContextProvider {
  _context: Promise<JsonLdContextNormalized>;

  /**
   * Creates a new resolver for the given context(s).
   */
  constructor(...contexts: JsonLdContext[]) {
    this._context = new ContextParser().parse([...contexts]);
  }
  ...
  async extendContext(...contexts: JsonLdContext[]) {
    this._context = this._context.then(context => new ContextParser().parse([context.getContextRaw(), ...contexts]))
  }
}
